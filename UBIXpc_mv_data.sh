...
for FILE in $FILES; do
  ...
  if ssh "$REMOTE_HOST" "docker exec $CONTAINER_ID test -f $DEST_FILE"; then
    ...
    scp "$SOURCE_FILE" "$REMOTE_HOST:$DEST_FILE"
    ssh "$REMOTE_HOST" "docker cp $DEST_FILE $CONTAINER_ID:$DEST_FILE"
  fi
done
...
#else
#    scp "$TAR_FILE" "$REMOTE_HOST:$DEST_TAR_FILE"
#    ssh "$REMOTE_HOST" "docker cp $DEST_TAR_FILE $CONTAINER_ID:$DEST_TAR_FILE"
...
#    scp /home/shiva/Desktop/leftImg8bit_trainvaltest.zip "$REMOTE_HOST:/DAFormer-SCDD/data/cityscapes"
#    scp /home/shiva/Desktop/gtFine_trainvaltest.zip "$REMOTE_HOST:/DAFormer-SCDD/data/cityscapes"
#    
#    ssh "$REMOTE_HOST" "docker exec -it $CONTAINER_ID bash -c '
